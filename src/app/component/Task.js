...
const Task = () => {
  ...
  const rowsLimit = 5;
  const [rowsToShow, setRowsToShow] = useState(product.slice(0, rowsLimit));
  const totalPages = Math.ceil(product.length / rowsLimit);
  ...
  const pageChange = (pageNo) => {
    setRowsToShow(product.slice((pageNo - 1) * rowsLimit, rowsLimit * pageNo));
  ...
  const renderPagination = () => {
    ...
    if (totalPages <= 3) {
      ...
      // Show the ellipsis and current/nearby pages if the current page is not near the beginning or end
      if (currentPage > 2) {
        pages.push("...");
      }
  ...
  return (
    <div>
      <table>
        ...
        <tbody>
          {rowsToShow?.map((apiData) => (
            <tr key={apiData.id}>
              <td style={{ paddingLeft: "10px" }}>{apiData.id}</td>
              <td style={{ paddingLeft: "10px" }}>{apiData.title}</td>
              <td
                style={{ paddingLeft: "10px", cursor: "pointer" }}
                onClick={() => openModal(apiData.id)}
              >
                Edit
                {isModalOpen && currentId === apiData.id && (
                  <Modal
                    product={product}
              ...
              <td
                style={{ paddingLeft: "10px", cursor: "pointer" }}
                onClick={() => openDeleteModal(apiData.id)}
              >
                Delete
                {isDeleteModalOpen && deleteModalId === apiData.id && (
                  <DeleteModal
                    product={product}
                    setProduct={setProduct}
                    setRowsToShow={setRowsToShow}
